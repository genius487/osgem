...
def postprocessing(res_df, df, sets_df, defaults_df):
    ...
    df2 = df_PBT.groupby(['REGION', 'YEAR', 'TECHNOLOGY', 'NAME']).sum()
    df2=df2.reset_index()
    namelist = []
    ...
    df2 = df_UBT.groupby(['REGION', 'YEAR', 'TECHNOLOGY', 'NAME']).sum()
    df2=df2.reset_index()
    namelist = []
    for i in range(0, len(df2)):
        ...
        for y in dfint['YEAR']:
            ...
            for i in range(0, len(vallist)):
                if int(y) - int(yearlist[i]) >=0 and int(y) - int(yearlist[i]) < oplife:
                    summation = summation + vallist[i]
            corvallist.append(summation)
    ...
    fixcost['YEAR'] = fixcost['YEAR'].astype(int)
    fixcost = fixcost[['YEAR', 'TECHNOLOGY', 'VALUE']].copy()
    newcap = res_df[res_df['NAME'] == 'NewCapacity']
    newcap['YEAR'] = newcap['YEAR'].astype(int)
    df_merge = pd.merge(fixcost, newcap, on=['YEAR', 'TECHNOLOGY'])
    ...
    df_merge = pd.merge(df_merge, dr_f, on=['REGION', 'TECHNOLOGY', 'YEAR'])
    df_merge['YEAR'] = df_merge['YEAR'].astype(int)
    df_merge['VALUE_r'] = (df_merge['VALUE_x'] + df_merge['VALUE_y']) * ( 1 / (1 + df_merge['VALUE'])**(df_merge['YEAR'] - min(df_merge['YEAR'])))
    df_merge = df_merge.drop(['VALUE_x', 'VALUE', 'VALUE_y', 'PARAM_x', 'PARAM_y'], axis=1)
    df_merge.rename(columns = {'VALUE_r':'VALUE' }, inplace = True)
    namelist = []
    for i in range(0, len(df_merge)):
