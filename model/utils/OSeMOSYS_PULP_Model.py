...
def OSeMOSYS_PULP_Model(inputFile, inputDir, solver, otoole, onsset_demand=None, demand_param="", fuel_name=""):
    if onsset_demand is None:
        onsset_demand = []
    if solver == 'cplex':
    ...
    if otoole is False:
        ...
        # For receiving the output of ONSSET for input into OSeMOSYS
        if onsset_demand != []:
            if fuel_name in df["FUEL"].to_list() and demand_param in ["AccumulatedAnnualDemand", "SpecifiedAnnualDemand"]:
                print(len(df.loc[(df['PARAM'] == demand_param) & (df['FUEL'] == fuel_name), 'VALUE']))
                print(len(onsset_demand))
                df.loc[(df['PARAM'] == demand_param) & (df['FUEL'] == fuel_name), 'VALUE'] = onsset_demand
                logging.info("OnSSET demand has been added to the model.")
            else:
                logging.info(
                    "Warning. OnSSET demand has NOT been added to the model. Please review fuel name or param name.")
        ...
        if onsset_demand != []:
            if fuel_name in FUEL['VALUE'].to_list() and demand_param in ["AccumulatedAnnualDemand",
                                                                         "SpecifiedAnnualDemand"]:
                if demand_param == "AccumulatedAnnualDemand":
                    AccumulatedAnnualDemand.loc[AccumulatedAnnualDemand['FUEL'] == fuel_name, 'VALUE'] = onsset_demand
                    logging.info("OnSSET demand has been added to the model.")
                else:
                    SpecifiedAnnualDemand.loc[SpecifiedAnnualDemand['FUEL'] == fuel_name, 'VALUE'] = onsset_demand
                    logging.info("OnSSET demand has been added to the model.")
            else:
                logging.info(
                    "Warning. OnSSET demand has NOT been added to the model. Please review fuel name or param name.")
